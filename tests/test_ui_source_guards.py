...
def test_m3d_schema_version_is_updated():
    ...
    assert 'PROJECT_SCHEMA_VERSION = "0.5.17-commercial-ui32c-compact-reversible-sdl-schedule"' in validation_src
...
def test_m3b_load_pages_use_editable_tables_and_code_basis():
    ...
    assert "render_arrow_free_csv_editor" in src
    assert "render_compact_sdl_component_table" in src
    assert "sdl_compact_" in src
...
def test_m3a_no_duplicate_sdl_summary_input_pattern():
    src = _src()
    assert 'D["load_components"]["sdl_components"] = structured_rows' in src
...
def test_m3e_wind_report_figures_and_auto_factor_ui_are_present():
    ...
    assert "wind_parameter_editor_" in src
    assert "Arrow-free input grid" in src
...
def test_m3g_coordinate_section_engine_ui_is_present():
    ...
    assert "Arrow-safe CSV editor" in src
...
def test_fea5b_uls_force_review_charts_and_nested_subpages():
    ...
    assert "Sections 6–7 remain disconnected" in src
    assert "transfer_component_figure" in src
    assert "Transfer-stage simultaneous force-vector table" in src
    assert "validated SINGLE STATE source vectors are preserved" in src
    requirements = (APP_SOURCE.parent / "requirements.txt").read_text(encoding="utf-8")
    assert "plotly==5.24.1" in requirements


def test_fea5c1_transfer_signed_governing_ui_semantics_are_explicit():
    src = APP_SOURCE.read_text(encoding="utf-8")
    figures_src = (APP_SOURCE.parent / "visualization" / "fea_figures.py").read_text(encoding="utf-8")
    assert 'meta["transfer_metric"]' in src
    assert 'f"{governing.get(\'value\', 0.0):,.2f} {meta[\'unit\']}"' in src
    assert "Absolute magnitude" in src
    assert '"Signed governing value", "Absolute magnitude"' in src
    assert '"transfer_governing": "Governing M3 (Mx)"' in figures_src
    assert 'name=meta["transfer_governing"]' in figures_src
    assert 'text=meta["transfer_governing"]' in figures_src


def test_fea5d1_final_service_component_review_is_primary_and_source_safe():
    src = APP_SOURCE.read_text(encoding="utf-8")
    figures_src = (APP_SOURCE.parent / "visualization" / "fea_figures.py").read_text(encoding="utf-8")
    readme = README_SOURCE.read_text(encoding="utf-8")
    assert "COMMERCIAL.FEA.5D1" in readme
    assert "_render_final_service_sls" in src
    assert "Final Service SLS force review" in src
    assert "Final Service SLS scalar-bound semantics" in src
    assert "SERVICE_COMPONENT_SECTIONS" in src
    assert '"summary": "Summary"' in src
    assert '"P": "P (Axial)"' in src
    assert '"M3": "M3 (Mx)"' in src
    assert "component-first mixed-source scalar-envelope review" in src
    assert "Sections 8–9 remain disconnected" in src
    assert "Governing Final Service scalar-force summary" in src
    assert "Detailed mixed-source scalar-bound audit" in src
    assert "_render_fea_service_scalar_audit" in src
    assert "_render_fea_envelope_stage" not in src
    assert "Final Service {meta['title']} scalar envelope table" in src
    assert "service_component_envelope_figure" in src
    assert "CSiBridge mixed-source scalar envelope" in figures_src
...
def test_pandas_arrow_string_native_crash_guard_is_pinned_and_explicit():
    app_src = Path("app.py").read_text(encoding="utf-8")
    requirements = Path("requirements.txt").read_text(encoding="utf-8")
    assert "pandas==2.3.3" in requirements
    assert "pandas>=2.2" not in requirements
    assert "pd.options.future.infer_string = False" in app_src
    assert "plotly==5.24.1" in requirements


def test_pandas_crash_guard_keeps_native_diagnostics_visible():
    app_src = Path("app.py").read_text(encoding="utf-8")
    assert "faulthandler.enable" in app_src
    assert "[SBGP RUNTIME] pandas=" in app_src



def test_fea5e1_qa_wording_and_single_state_trace_closeout_workspace():
    src = _src()
    readme = README_SOURCE.read_text(encoding="utf-8")
    validation_src = VALIDATION_SOURCE.read_text(encoding="utf-8")
    ...
    assert "Downstream Readiness" in src
    assert "package_integrity_gates" in src
    assert "trace_component_extremum" in src
    assert "SOURCE READY is not a design PASS" in src
    assert "Compact envelope → original source row" in src
    assert "SIMULTANEOUS SOURCE VECTOR" in src
    assert "NOT A SIMULTANEOUS FORCE VECTOR" in src
    assert "Gate severity describes the consequence only if a gate fails" in src
    assert '"Gate severity": "BLOCKING" if blocking else "ADVISORY"' in (APP_SOURCE.parent / "core" / "fea_results.py").read_text(encoding="utf-8")
    assert '"Blocking": "YES"' not in (APP_SOURCE.parent / "core" / "fea_results.py").read_text(encoding="utf-8")
    assert "Section 9 additionally needs a displacement source" in src
    assert "SOURCE MISSING" in src
    requirements = (APP_SOURCE.parent / "requirements.txt").read_text(encoding="utf-8")
    assert "pandas==2.3.3" in requirements
    assert "plotly==5.24.1" in requirements


def test_ui32c_compact_sdl_schedule_is_reversible_and_arrow_safe():
    src = _src()
    assert "render_compact_sdl_component_table" in src
    assert 'labels = ["Component", "Single Track (kN/m)", "Double Track (kN/m)"]' in src
    assert "＋ Add SDL component" in src
    assert "Add project-specific SDL component" in src
    assert "Manage project-specific SDL components" in src
    assert "Archive removes a custom row from the active SDL total without deleting it" in src
    assert "Standard BG40 rows cannot be archived" in src
    assert "Calculated total" in src
    assert 'button("×"' not in src
    assert "Advanced CSV import / export" not in src
    assert "st.data_editor(" not in src
...
def test_ui32a_coordinate_csv_uses_stable_float_format():
    src = _src()
    assert 'float_format="%.4f"' in src
