...
def test_m3d_schema_version_is_updated():
    ...
    assert 'PROJECT_SCHEMA_VERSION = "0.5.12-commercial-fea5e-qa-source-trace-downstream-readiness"' in validation_src
...
def test_fea5e_qa_source_trace_and_downstream_readiness_workspace():
    src = _src()
    readme = README_SOURCE.read_text(encoding="utf-8")
    validation_src = VALIDATION_SOURCE.read_text(encoding="utf-8")
    assert "COMMERCIAL.FEA.5E" in readme
    assert 'PROJECT_SCHEMA_VERSION = "0.5.12-commercial-fea5e-qa-source-trace-downstream-readiness"' in validation_src
