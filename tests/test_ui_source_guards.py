...
def test_m3d_schema_version_is_updated():
    ...
    assert 'PROJECT_SCHEMA_VERSION = "0.5.2-commercial-fea5b-uls-force-review-charts"' in validation_src
...
def test_fea5b_uls_force_review_charts_and_nested_subpages():
    ...
    report_schema_src = REPORT_SCHEMA_SOURCE.read_text(encoding="utf-8")
    defaults_src = DEFAULTS_SOURCE.read_text(encoding="utf-8")
    assert "COMMERCIAL.FEA.5B" in readme
    assert 'PROJECT_SCHEMA_VERSION = "0.5.2-commercial-fea5b-uls-force-review-charts"' in validation_src
    ...
    assert "5.2.1 Axial Force P" in src
    assert "5.2.2 Vertical Shear V2" in src
    assert "5.2.3 Torsion T" in src
    assert "5.2.4 Bending Moment M3" in src
