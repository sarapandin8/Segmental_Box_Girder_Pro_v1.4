...
def test_m3d_schema_version_is_updated():
    ...
    assert 'PROJECT_SCHEMA_VERSION = "0.5.10-commercial-fea5d1b-upload-widget-session-reset-hotfix"' in validation_src
...
def test_project_json_load_uses_pending_state_before_widget_keys():
    ...
    load_handler = src.split('"Load uploaded project",', 1)[1].split('if st.session_state.get("project_load_message")', 1)[0]
...
def test_fea5d1b_upload_widgets_are_schema_scoped_and_project_loader_is_on_demand():
    src = _src()
    assert 'def _versioned_upload_key' in src
    assert 'Open Project JSON loader' in src
    assert 'project_json_loader_visible' in src
    assert 'key=_versioned_upload_key("project_json_upload", epoch=upload_epoch)' in src
    assert 'raw_project_json = uploaded.getvalue()' in src
    assert src.index('raw_project_json = uploaded.getvalue()') > src.index('"Load uploaded project",')
    assert 'key=_versioned_upload_key("section_coordinate_file_upload")' in src
    assert 'key=_versioned_upload_key("tendon_general_upload")' in src
    assert 'key=_versioned_upload_key(f"fea_{stage}_workbook_upload")' in src
    assert (APP_SOURCE.parent / "sitecustomize.py").exists()
