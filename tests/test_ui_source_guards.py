...
def test_m3b_load_pages_use_editable_tables_and_code_basis():
    ...
    assert "render_arrow_free_sdl_component_table" in src
    assert "render_sdl_advanced_csv_tools" in src
    assert "sdl_structured_" in src
...
def test_ui32a_structured_sdl_editor_avoids_arrow_and_exposes_bulk_csv_tools():
    src = _src()
    assert "Structured SDL component table" in src
    assert "Add SDL component" in src
    assert "Advanced CSV import / export" in src
    assert "Apply CSV to table" in src
    assert "st.data_editor(" not in src
